...
class TapasScraper(BaseScraper):
    ...
    proxy_domains = [
        "tapas.io",
        "www.tapas.io",
        "us-a.tapas.io",  # CDN régional des covers (og:image)
        "s3.tapasticusercontent.com",
        "tapas-prod.s3.amazonaws.com",
    ]
    ...
    def _search(self, session, terms: str) -> List[dict]:
        res = _throttled_get(
            self, session, f"{_BASE}/search", params={"q": terms, "t": "SERIES"}, timeout=25
        )
        if res.status_code != 200:
            # fallback sans t=
            res = _throttled_get(
                self, session, f"{_BASE}/search", params={"q": terms}, timeout=25
            )
        if res.status_code != 200:
            return []
